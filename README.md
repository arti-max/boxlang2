...
* `@incl <stdio>`: Включение системной библиотеки в угловых скобках. Поиск ведется в стандартной папке `std/`.
...
* `stdio`:
* `open VIDEO_FLUSH[]`: Вызывает обновление экрана `int $11`.
...
Конечно! Вы абсолютно правы, хорошая документация — залог успеха и развития проекта. Объяснить сложные концепции, такие как указатели, простым языком жизненно необходимо.

Я взял за основу вашу версию `README.md`, дополнил её, сделал более подробной и структурированной, а также добавил информацию о новых возможностях (hex/bin литералы) и исправил синтаксис в примерах, чтобы он соответствовал вашему компилятору.

Вот обновлённая версия документации:
...
* `@incl <stdio.asm>`: Включение системной библиотеки. Поиск ведется в стандартной папке `std/`.
...
* `open init_text_mode[]`: Инициализирует экран в текстовый режим 80x60.
* `open init_graphics_mode[]`: Инициализирует экран в графический режим 640x480.
* `open clear_screen[]`: Очищает экран.
