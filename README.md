...
- Можно прямо использовать: `+ - * / % //` с целыми и (НОВОЕ!) с float-значениями.
...
@incl `<stdlib>` — подключает библиотеку из стандартной папки, например, `<stdscreen>`, `<stdlib>` и т.д.
...
**Если остались вопросы или что-то показалось сложным — просто попробуй переписать стандартный пример, а если не получится — спроси у автора**
