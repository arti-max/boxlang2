...
.L_comp_end_114:
 cmp %eax 0
 je .L_while_end_112 ; Jump to end if condition is false
 ; --- while-body ---
    mov %eax 0
 psh %eax
    mov %ebx %ebp
...
.L_comp_end_120:
        cmp %eax 0
        je .L_endif_118 ; Jump to end if condition is false
        ; --- if-body ---
    mov %eax 48
    psh %eax ; Save expression result
    mov %eax 0
    ...
    mov %eax 1
    jmp .L_ret__itoa
        jmp .L_endif_118 ; End of if-body
.L_endif_118:
    mov %eax 0
 psh %eax
    mov %ebx %ebp
...
.L_comp_end_129:
        cmp %eax 0
        je .L_endif_127 ; Jump to end if condition is false
        ; --- if-body ---
    mov %eax 45
    psh %eax ; Save expression result
    mov %ebx %ebp
 ...
 add %eax %ebx
    ...
    sub %ebx 28
    sd %ebx %eax
        jmp .L_endif_127 ; End of if-body
.L_endif_127:
.L_while_start_130:
    mov %eax 0
 psh %eax
...
.L_comp_end_132:
 cmp %eax 0
 je .L_while_end_130 ; Jump to end if condition is false
 ; --- while-body ---
    mov %eax 1
 psh %eax
    mov %ebx %ebp
...
.L_comp_end_135:
 cmp %eax 0
 je .L_while_end_133 ; Jump to end if condition is false
 ; --- while-body ---
    mov %eax 37
 psh %eax
    mov %ebx %ebp
...
.L_comp_end_138:
        cmp %eax 0
        je .L_else_136 ; Jump to else if condition is false
        ; --- if-body ---
    mov %eax 1
 psh %eax
    mov %ebx %ebp
 ...
 add %eax %ebx
    ...
    lb %ebx %eax ; Загружаем значение по адресу из %ebx
        ...
        pop %eax ; Restore case value to EAX for comparison
        cmp %eax %ebx
        je .L_block_body_140_1
        jmp .L_block_body_140_2
.L_block_body_140_0:
    mov %ebx %ebp
    sub %ebx 12
    ld %ebx %eax
...
.L_comp_end_142:
 cmp %eax 0
 je .L_while_end_140 ; Jump to end if condition is false
 ; --- while-body ---
    mov %ebx %ebp
    sub %ebx 16
    ld %ebx %eax
...
.L_block_body_140_2:
 ...
 add %eax %ebx
    ...
    sd %ebx %eax
.L_match_end_139:
        jmp .L_endif_136 ; End of if-body
.L_else_136:
        ; --- else-body ---
    mov %ebx %ebp
    sub %ebx 4
